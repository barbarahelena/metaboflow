...
classify_bins <- function(tiara, contig2bin, join_prokaryotes, assembler){
    ...
    tiara_with_bins <- tiara |>
        left_join(contig2bin) |>
        filter(!is.na(BinID))
    
    # Transform data based on input format
    tiara_transformed <- if("class_fst_stage" %in% colnames(tiara_with_bins)) {
        # For classification format, we need to handle this differently
        # Each row represents a contig with its classification
        tiara_with_bins |>
            # Create probability-like columns based on classification
            mutate(
                Archaea = if_else(class_fst_stage == "archaea", 1, 0),
                Bacteria = if_else(class_fst_stage == "bacteria", 1, 0),
                Eukarya = if_else(class_fst_stage == "eukarya", 1, 0),
                Organelle = if_else(class_fst_stage == "organelle", 1, 0),
                Unknown = if_else(class_fst_stage == "unknown", 1, 0)
            ) |>
            select(sequence_id, BinID, Archaea, Bacteria, Eukarya, Organelle, Unknown)
    } else {
        # Original probability format
        probability_data <- tiara_with_bins |>
            select(sequence_id,
                   BinID,
                   Archaea = arc,
                   Bacteria = bac,
    ...
    final_tiara <- if(join_prokaryotes) {
        tiara_transformed |>
    ...
    ## Calculate probabilities based on summed bin probabilities for each category
    bin_probabilities <- final_tiara |>
        group_by(BinID) |>
        summarise(across(all_of(prob_columns), sum), .groups = "drop")
    
    normalized_probabilities <- bin_probabilities |>
        rowwise() |>
        mutate(
            denominator = sum(c_across(all_of(prob_columns))),
            across(all_of(prob_columns), \(x) if_else(denominator > 0, x/denominator, 0)),
    ...
    unclassified_bins <- all_bins[!(all_bins %in% normalized_probabilities$BinID)]

    ## Assign unclassified bins
    final_probabilities <- if(length(unclassified_bins) > 0) {
        unclassified_bins_tbl <- if(join_prokaryotes == TRUE) {
            tibble(
        ...
        bind_rows(normalized_probabilities, unclassified_bins_tbl)
    } else {
